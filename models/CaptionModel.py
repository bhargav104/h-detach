...
class CaptionTwinParamModel(nn.Module):
    def __init__(self, opt, reverse = False):
        super(CaptionTwinParamModel, self).__init__()
        self.vocab_size = opt.vocab_size
        self.input_encoding_size = opt.input_encoding_size
        self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        ...
        self.dropout = nn.Dropout(self.drop_prob_lm)
        if not self.reverse:
            self.ln_hidden = nn.Linear(self.rnn_size, self.rnn_size) # linear transform on hidden units
...
class CaptionTwinModel(nn.Module):
    def __init__(self, opt, reverse = False):
        super(CaptionTwinModel, self).__init__()
        self.vocab_size = opt.vocab_size
        self.input_encoding_size = opt.input_encoding_size
        self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
...
class ShowAttendTellTwinCore(nn.Module):
    def __init__(self, opt, reverse=False):
        super(ShowAttendTellTwinCore, self).__init__()
        self.input_encoding_size = opt.input_encoding_size
        self.rnn_type = opt.rnn_type
        self.rnn_size = opt.rnn_size
        self.num_layers = opt.num_layers
    ...
    def forward(self, xt, fc_feats, att_feats, state):
        ...
        (h ,c) = state
        p_val = np.random.random(size=1)[0]
        if self.opt.p_detach != 0.0 and p_val <= self.opt.p_detach:
            h = h.detach()
        state = (h ,c)
...
class ShowAttendTellTwinModel(CaptionTwinModel):
    def __init__(self, opt, reverse=False):
        super(ShowAttendTellTwinModel, self).__init__(opt, reverse=reverse)
        self.core = ShowAttendTellTwinCore(opt, reverse=reverse)

class ShowAttendTellTwinParamModel(CaptionTwinParamModel):
    def __init__(self, opt, reverse=False):
        super(ShowAttendTellTwinParamModel, self).__init__(opt, reverse=reverse)
        self.core = ShowAttendTellTwinCore(opt, reverse=reverse)
